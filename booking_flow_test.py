...
today = date.today().strftime("%Y|%m|%d")       # 取得今天的日期


def ask_booking_information():
    print("Ask booking information")

    user_response = input("請輸入你的高鐵訂位資訊，包含: 出發站、到達站、出發日期、出發時刻:")

    system_prompt = f"""
    我想要從回話取得訂票資訊，包含: 出發站、到達站、出發日期、出發時刻，
    今天是 {today}，請把資料整理成python dictionary格式，例如：{standard_format}，
    請把資料整理成python dictionary格式，不包含其他內容。
    """
...
def ask_missing_information(booking_info):      # Slot filling
    ...
    if not missing_slots:
        ...
        user_response = input(f"請補充你的高鐵訂位資訊，包含{','.join(missing_slots)}:")
